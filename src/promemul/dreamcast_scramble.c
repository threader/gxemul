 ...
 *  Marcus software page.
 *
 *  The only modification done to make it fit into the emulator is that
 *  it is executed internally from src/file.c.
 */
...
int main(int argc, char *argv[])
{
  ...
  if(opt)
    dreamcast_descramble(argv[2], argv[3]);
  else
